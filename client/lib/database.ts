...
export class DatabaseService {
  ...
  static async completeOnboarding(userId: string, profileData: Partial<UserProfile>): Promise<void> {
    await supabase
      .from('user_profiles')
  ...
  static async upgradeUserPackage(userId: string, packageId: string, paymentData?: any): Promise<UserSubscription> {
  ...
  static async createResume(userId: string, resumeData: Partial<Resume>): Promise<Resume> {
    const { data } = await supabase
      .from('resumes')
      .insert({
        user_id: userId,
  ...
  static async updateResume(resumeId: string, resumeData: Partial<Resume>): Promise<Resume> {
    const { data } = await supabase
      .from('resumes')
      .update(resumeData)
      .eq('id', resumeId)
  ...
  static async createResumeRevision(resumeId: string, userId: string, changes: any): Promise<ResumeRevision> {
  ...
  // Job Matching
  static async getUserJobMatches(userId: string, limit?: number): Promise<JobMatch[]> {
    let query = supabase
      .from('job_matches')
      .select('*')
      .eq('user_id', userId)
  ...
  static async saveJobMatch(userId: string, jobData: Partial<JobMatch>): Promise<JobMatch> {
    const { data } = await supabase
      .from('job_matches')
      .insert({
        user_id: userId,
  ...
  static async incrementUsageStats(userId: string, field: 'resumes_created' | 'revisions_used' | 'jobs_viewed'): Promise<void> {
  ...
  static async createSampleResume(userId: string): Promise<string> {
    const { data, error } = await supabase.rpc('create_sample_resume_for_user', {
      user_uuid: userId
    })
...
export async function checkUserLimits(userId: string) {
  ...
  return {
    packageLimits,
    currentUsage,
    canCreateResume: packageLimits.resume_limit === -1 || currentUsage.resumes_created < packageLimits.resume_limit,
    canMakeRevision: packageLimits.revision_limit === -1 || currentUsage.revisions_used < packageLimits.revision_limit,
    canViewJobs: packageLimits.job_match_limit === -1 || currentUsage.jobs_viewed < packageLimits.job_match_limit
  }
}
